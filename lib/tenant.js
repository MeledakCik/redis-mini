...
function buildUsername(ownerId) {
...
async function applyAclUser({ username, password }) {
...
async function removeAclUser(username) {
  try {
    const admin = getAdminRedis();
...
async function purgeTenantKeys(username) {
  const admin = getAdminRedis();
