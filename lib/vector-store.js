...
//
// Task fix (port conflict 6333): instance lama yang sempat kesimpen dengan host: "external",
// port: null (bug lama waktu provider.mode === "external") dibetulin di sini juga, biar
// connection string yang ditampilkan gak lagi "qdrant://default:token@external:null/name".
function migrateVectorInstance(inst) {
  let fixed = inst.provider ? inst : { ...inst, provider: "docker" };
  if (!fixed.host || fixed.host === "external" || !fixed.port) {
    const { host, port } = resolvePublicQdrantEndpoint();
    fixed = { ...fixed, host, port };
  }
  return fixed;
}
