...
export const { handlers, auth, signIn, signOut } = NextAuth({
      ...
      token: "https://github.com/login/oauth/access_token",
        ...
        async request({ tokens, provider }) {
          const res = await fetch("https://api.github.com/user", {
            headers: { Authorization: `Bearer ${tokens.access_token}`, "User-Agent": "redis-mini" }
          });
          const profile = await res.json();
          // kalo email private, fetch dari /user/emails
          if (!profile.email) {
  ...
  callbacks: {
    async signIn({ user, account }) {
      if (account?.provider === "google" || account?.provider === "github" || account?.provider === "github-custom") {
        if (!isEmailWhitelisted(user?.email)) return "/unauthorized";
      }
      return true;
    },
