...
import { IS_DOCKER_AVAILABLE, DEPLOYMENT_MODE } from "@/lib/env";
...
const DockerProvider = {
  ...
  async createRedisInstance({ id, port, password, containerName }) {
    ...
    const container = await d.createContainer({
      name: containerName,
      Image: REDIS_IMAGE,
      Labels: { [LABEL_KEY]: LABEL_VALUE, "mini-upstash.id": id },
      Cmd: [
        "redis-server",
  ...
  // --- Qdrant (vector backend), satu container dipakai bersama semua vector DB ---
  ...
  async getVectorBackendStatus() {
    const d = await getDocker();
    try {
      const info = await d.getContainer(QDRANT_CONTAINER_NAME).inspect();
  ...
  async getVectorBackendMemoryBytes() {
    try {
      ...
      if (!status.running || !status.containerId) return null;
      return (await this._containerStats(status.containerId)).memory_stats?.usage || null;
    } catch {
      return null;
    }
  },

  qdrantBaseUrl: `http://127.0.0.1:${QDRANT_PORT}`,
};
...
const ExternalProvider = {
  ...
  async getVectorBackendStatus() {
    return { running: !!process.env.QDRANT_URL, containerId: null };
  },
...
// kebetulan lagi jalan di mode yang lain (mis. migrasi data lama dari Docker ke external).
export function getProviderForInstance(inst) {
  return inst?.provider === "external" ? ExternalProvider : DockerProvider;
}

export { QDRANT_PORT, IS_DOCKER_AVAILABLE, DEPLOYMENT_MODE };
