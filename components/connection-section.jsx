...
function CopyField({ label, value, mono = true, icon: Icon }) {
  ...
  const canMask = label.toLowerCase().includes("password") || label.toLowerCase().includes("token") || label.toLowerCase().includes("connection") || label.toLowerCase().includes("cli");
  const display = canMask && !revealed ? value.replace(/(:\/\/default:)([^@]+)(@)/, (m, a, pass, c) => `${a}${"•".repeat(Math.min(pass.length, 16))}${c}`) : value;
